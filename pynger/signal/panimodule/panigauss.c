...
static void f_iir_xline_filter(SRCTYPE *src, DSTTYPE *dest, int sx, int sy,
		double *filter);
static int f_iir_yline_filter(DSTTYPE *src, DSTTYPE *dest, int sx, int sy,
		double *filter);
static int f_iir_tline_filter(DSTTYPE *src, DSTTYPE *dest, int sx, int sy,
		double *filter, double tanp);
static int f_iir_derivative_filter(DSTTYPE *src, DSTTYPE *dest, int sx, int sy,
	...
	double sigmav, double sigmau, double phi, int orderv, int orderu)
{
		...
		if (tanp != 0.0) {
				...
				if (f_iir_tline_filter(output,output,sizex,sizey,filter, tanp)){
					return -1;
				}
		}
		else {
				/* isotropic filter or anisotropic filter aligned with grid */
				if (f_iir_yline_filter(output,output,sizex,sizey,filter)){
					return -1;
				}
		}
		...
		for(i=0; i<orderv; i++){
				if(f_iir_derivative_filter(output, output, sizex, sizey, phirad-PI/2., 1)){
					return -1;
				}
		}
		for(i=0; i<orderu; i++){
				if(f_iir_derivative_filter(output, output, sizex, sizey, phirad, 1)){
					return -1;
				}
		}
